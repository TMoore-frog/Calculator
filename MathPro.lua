...
function goupdate()
    ...
    downloadUrlToFile(updatelink, thisScript().path, function(id3, status1, p13, p23) -- качает ваш файлик с latest version
        if status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
            print(('{02FFAB}[Konstant+EFK]{FFFFFF} Обновление завершено!'), color)
                thisScript():reload()
        end
    end)
end
...
function update()
    ...
    downloadUrlToFile('https://api.jsonbin.io/b/5fcb9596516f9d1270286355/1', fpath, function(id, status, p1, p2) -- ссылку на ваш гитхаб где есть строчки которые я ввёл в теме или любой другой сайт
